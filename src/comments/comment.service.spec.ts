  ...
  afterEach(() => {
    ...
    it('should create a comment without task', async () => {
      const commentDto = {
        content: 'New Comment',
        userName: 'user1',
        taskId: null,
      ...
      const user = { id: 1, userName: 'user1' };
      const newComment = { ...commentDto, user };
      const savedComment = { id: 1, ...newComment };
    ...
    it('should throw ConflictException when save fails', async () => {
      ...
      mockCommentRepository.create.mockReturnValue(newComment);
      mockCommentRepository.save.mockRejectedValue({ detail: 'Error detail' });
      mockUserService.findByUserName.mockResolvedValue(user);
      mockTasksService.findOne.mockResolvedValue(task);
    
      await expect(service.create(commentDto)).rejects.toThrow(ConflictException);
      expect(mockCommentRepository.create).toHaveBeenCalledWith(commentDto);
      expect(mockUserService.findByUserName).toHaveBeenCalledWith('user1');
      expect(mockTasksService.findOne).toHaveBeenCalledWith(1);
      expect(mockCommentRepository.save).toHaveBeenCalledWith(newComment);
    ...
    it('should create a comment without user when userName is not provided', async () => {
      const commentDto = {
        content: 'New Comment',
        taskId: 1,
        userName: undefined // Asignamos undefined explícitamente
      };
      const task = { id: 1, title: 'Task 1' };
      const newComment = { content: commentDto.content, task };
      const savedComment = { id: 1, ...newComment };
  ...
  describe('update', () => {
    it('should update an existing comment', async () => {
      ...
      it('should throw ConflictException when update fails', async () => {
        ...
        mockCommentRepository.findOneBy.mockResolvedValue(existingComment);
        mockUserService.findByUserName.mockResolvedValue({ id: 2, userName: 'user2' });
        mockCommentRepository.merge.mockReturnValue({...existingComment, ...commentDto});
        mockCommentRepository.save.mockRejectedValue({ detail: 'Error detail' });
      
        await expect(service.update(id, commentDto)).rejects.toThrow(ConflictException);
      });
